...
public class ReportActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!dataIntent.getBooleanExtra("isEdit", false))
            manifestGenericUI();
        else {
            reportListModel = (ReportListModel) dataIntent.getSerializableExtra("data");
            try {
    ...
    private void manifestGenericUI() {
        try {
            JSONArray inputJson = JsonHelper.getJsonData(this);
            for (int i = 0; i < inputJson.length(); i++) {
                ...
                if (object.getString(JsonHelper.TYPE_KEY).equalsIgnoreCase(TYPE_DROPDOWN)) {
                    genericDropDown(reportdataModel, i);
                } else {
                    genericEditText(reportdataModel, i);
                }
            }
            reportListModel.setReportDataModelList(reportDataModelsList);
        } catch (JSONException e) {
            Log.e(LOG_TAG, e.getMessage());
        }
    }
